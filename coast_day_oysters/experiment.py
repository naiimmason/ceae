...
data_1path = "db/redundancy-data-" + time + ".csv"
survey_1path = "db/redundancy-survey-" + time + ".csv"
...
data_file.write("date, subject, treatment, final option selected, number of oysters, cooked option, " + \
    "question number, question text, price, answer, " + \
    option_names + "\n")
data_file.close()

redun_data_file = open(data_1path, "w")
redun_data_file.write("Date," + time + "\n")
redun_data_file.write("date, subject, treatment, number of oysters, " + \
    "question number, question text, price, answer\n")
redun_data_file.close()

redun_survey_file = open(survey_1path, "w")
redun_survey_file.write("Date," + time + "\n")
redun_survey_file.write("date, subject_id, ")
redun_survey_file.write(option_names + "\n")
redun_survey_file.close()
...
def session(me):
  ...
  if subj_id == "econadmin012":
    ...
    subject.start(me, subj_id, data_filepath, data_1path, survey_1path)
