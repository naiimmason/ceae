...
def experiment2(me, subj_id, num_oysters):
  ...
  if rand_questions == None:
    rand_questions = []
    new_questions = take({"tag": "new_questions"})["qs"]
    rand.shuffle(new_questions)

    # pop off the randomly shuffled questions and add to the new list
    for i in range(num_options):
      if(len(new_questions) != 0):
        rand_questions.append(new_questions.pop())

    # Regenerate the list of questions if they are gone
    if len(new_questions) == 0:
      new_questions = list(worded_questions)
      rand.shuffle(new_questions)

    if len(rand_questions) != num_options:
      num_left = num_options - len(rand_questions)
      for i in range(num_left):
        rand_questions.append(new_questions.pop())

    rand.shuffle(rand_questions)
    put({"tag": "new_questions", "qs": new_questions})
